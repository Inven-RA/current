...
const MyComponents = { 
    ...
    IapActivities: function DatePicker(props) {
      ...
      return <div> <Navbar bg="blue" variant="green" style={{ marginBottom: '1em' }}>
            <Nav >
                <div class="two">
                    <h1>Learner {userID}</h1>
                </div>
            </Nav>
        </Navbar>
        <hr></hr>
        <BackButton/>
        <h4 class="subTitle"  style={{marginTop:'1em'}}>IAP's Progress:</h4>
        </div>     
      ;
    },
            ...
            for ( var i = 0; i < correspondent_analytics.length; i++) {
                for ( var j = 0; j < correspondent_scores.length; j++) {
                    if (correspondent_scores[j].user_id == user_ID){
                        if (correspondent_scores[j].analytics_id == correspondent_analytics[i].id){
                            value += correspondent_scores[j].score/100 * objectivesAnalytics[i].weight;
                        }
                    }
                } 
            }
    ...
    IapDetails: function DatePicker(props) {
      ...
      useEffect(() => {
      // Perform the requests in parallel using Promise.all()
      Promise.all([axios.get('/objectives'), axios.get('/iaps'), axios.get('/allAnalytics'), axios.get('/objectivesAnalytics')])
          .then(responses => {
          const objectives = responses[0].data;
          const iaps = responses[1].data;
          const allAnalytics = responses[2].data;
          const objectivesAnalytics = responses[3].data;
      ...
      var iaps_sum = 0;
      for (var q = 0; q < iaps.length; q++){

        //ver quais os objetivos desta iap_ID
        var correspondent_objectives = [];
        var correspondent_objectives_ids = [];
        for (var o = 0; o < objectives.length; o++) {
            if (objectives[o].iap_id == iaps[q].id){
                correspondent_objectives.push(objectives[o]);
                correspondent_objectives_ids.push(objectives[o].id);
            }
        }
      ...
      var iaps_percentage = Math.round(iaps_sum/iaps.length);
      var variant3 = "success";
      if (iaps_percentage < 50) {
          variant3 = "danger";
      } else if (iaps_percentage < 75) {
          variant3 = "warning";
      }
      ...
      return ( 
          <div>
          <ProgressBar variant={variant3} now={iaps_percentage} label={`${iaps_percentage}%`}>
            <div className={`progress-bar variant-bg${variant3}`} 
                    style={{  width: iaps_percentage < 10 ? '20px' : `${iaps_percentage}%`,
                            backgroundColor: variant3 == 'success' ? '#3cb371': variant3 == 'warning' ? '#f5c71a' : '#cb4154'}}>
                {iaps_percentage}
            </div>
          </ProgressBar>
          <div className='iapTotal'>📜 <span style={{fontWeight:'bold'}}>{iaps_percentage}%</span> of IAP's completed </div>
          {iaps.map((row, index) => (
                <div class="analytics">
                <hr/>
                  <div class="activityTitle">
                  ...
                  <div >
                  <button class="button-4"
                                style={{ marginTop:'1em', marginBottom:'1em', marginLeft: '0'}}
                                onClick={() => handleBtnClick(index)}>
                        {isContentVisible === index ? 'HIDE' : 'SHOW'}
                        </button>
                        {isContentVisible === index && <MyComponents.IapActivitiesDetails iap_ID={row.id} />}
                  </div>
                </div>
